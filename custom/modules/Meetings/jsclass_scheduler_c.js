...
SugarWidgetSchedulerAttendees.form_add_attendee_c = function (contactSelected) {
    if(typeof ([contactSelected]) != 'undefined' && typeof(GLOBAL_REGISTRY.focus.users_arr_hash[ [contactSelected].value]) == 'undefined') {

        var arrayLength = GLOBAL_REGISTRY.focus.users_arr.length;
        GLOBAL_REGISTRY.focus.users_arr[ arrayLength ] = {contactSelected, module:"Contact"};
        GLOBAL_REGISTRY.focus.users_arr[ arrayLength ].fields = {id:contactSelected.value, full_name:contactSelected.label};

    }
    console.log(GLOBAL_REGISTRY.focus.users_arr[ arrayLength]);

    //display is done in the contacts_c change function
    //GLOBAL_REGISTRY.scheduler_attendees_obj.display();
}
...
SugarWidgetScheduleRow.prototype.load = function (thetableid) {
    ...
    if (typeof (this.focus_bean.fields) != 'undefined') {
...
SugarWidgetScheduleRow.prototype.display = function() {
    SUGAR.util.doWhen("document.getElementById('" + this.thetableid + "') != null", function(){
        ...
        console.log("ScheduleRow.prototype.display:");
...
SugarWidgetScheduleRow.deleteRow = function(bean_id) {
    ...
    for(var i=0;i<GLOBAL_REGISTRY.focus.users_arr.length;i++) {
        if(GLOBAL_REGISTRY.focus.users_arr[i]['fields']['id']==bean_id) {
            ...
            var $selected = $('option:selected[value="'+bean_id+'"]');
...
SugarWidgetScheduleRow.deleteRow_c = function(bean_id) {
    // can't delete organizer
    /*
     if(GLOBAL_REGISTRY.focus.users_arr.length == 1 || GLOBAL_REGISTRY.focus.fields.assigned_user_id == bean_id) {
     return;
